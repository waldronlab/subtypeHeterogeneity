...
```{r scHeatmap}
sind <- 2:ncol(sc.expr)
scHeatmap(sc.expr[,sind], consensus.subtypes$consensusOV.subtypes[sind])
```
...
```{r marginCompTCGA}
m99 <- c(0.0000,  0.0570,  0.1100,  0.1128,  0.1490,  0.5280)
m92 <- c(0.00000, 0.02250, 0.03600, 0.05909, 0.06200, 0.52800)
tcga.rseq <- c(0.0040,  0.1600,  0.3040,  0.3332,  0.4760,  0.8760)
tcga.ma <- c(0.0000,  0.1325,  0.2870,  0.3199,  0.4870,  0.8520)
names(m99) <-c("Min.", "1st Qu.",  "Median", "Mean", "3rd Qu.",    "Max.") 
names(m92) <- names(tcga.rseq) <- names(tcga.rseq) <- names(m99)    
boxplot(m92, m99, tcga.rseq, tcga.ma, ylab="margin score", 
    names=c("scRNAseq92", "scRNAseq99","tcgaRNAseq", "tcgaMarray"), 
    col=c(rep(cb.red,2), rep(cb.blue,2)))
```
