...
```{r buildExtTraining}
#load("~/Documents/PostDoc/NewYork/ovc/esets.not.rescaled.classified.RData")
selectGenes <- function(eset)
{
    ind <- fData(eset)$gene %in% verhaak800
    return(eset[ind,]) 
}
#esets <- lapply(esets.not.rescaled.classified, selectGenes)
eset.file <- file.path(data.dir, "esets.not.rescaled.classified.filtered.RData")
